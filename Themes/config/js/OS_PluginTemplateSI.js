...
$(document).ready(function () {
    ...
    nbxget('os_plugintemplatesi_getdata', '#selectparams', '#editdata');

    $('.actionbuttonwrapper #cmdsave').unbind('click');
    $('.actionbuttonwrapper #cmdsave').click(function () {
        $('.processing').show();
        nbxget('os_plugintemplatesi_savedata', '#editdata');
    });
    ...
    $('.actionbuttonwrapper #cmddelete').click(function () {
        if (confirm($('#deletemsg').val())) {
            $('.processing').show();
            nbxget('os_plugintemplatesi_deleterecord', '#editdata');
        }
    });
    ...
    $('.selecteditlanguage').click(function () {
        ...
        nbxget('os_plugintemplatesi_selectlang', '#editdata'); // do ajax call to save current edit form
    });
...
function nbxgetCompleted(e) {
    ...
    if (e.cmd == 'os_plugintemplatesi_deleterecord') {
        $('#selecteditemid').val(''); // clear selecteditemid, it now doesn;t exists.
        nbxget('os_plugintemplatesi_getdata', '#selectparams', '#editdata');// relist after delete
    }

    if (e.cmd == 'os_plugintemplatesi_savedata') {
        $('#selecteditemid').val(''); // clear sleecteditemid.        
        nbxget('os_plugintemplatesi_getdata', '#selectparams', '#editdata');// relist after save
    }

    if (e.cmd == 'os_plugintemplatesi_selectlang') {
        $('#editlang').val($('#nextlang').val()); // alter lang after, so we get correct data record
        nbxget('os_plugintemplatesi_getdata', '#selectparams', '#editdata'); // do ajax call to get edit form
    }

    if (e.cmd == 'os_plugintemplatesi_getdata') {
